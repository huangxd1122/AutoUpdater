...
namespace AutoUpdater.ViewModels
{
    internal class MainWindowViewModel : BindableBase
    {
        ...
        private const string SubKey = @"HKEY_LOCAL_MACHINE\SOFTWARE\HXD\智能服务控制系统";
        ...
        private void OnDownloadFileCompleted(object sender, AsyncCompletedEventArgs e)
        {
            ...
            try
            {
                Process.Start(UpdateInfo.UnpackPath + "/com.hxd.samplemanage.exe");
            }
            catch (Exception ex)
            {
                StatusDescription = "主程序启动失败！";
                Loger.Print("Run application Err. ");
                return;
            }
        ...
        private void UpdateVersionFile(string newVersion)
        {
            string jsonstr = "";
            using (var item = File.OpenRead(AppDomain.CurrentDomain.BaseDirectory + "Versionfile.json"))
            {
                byte[] filestreem = new byte[item.Length];
                if (filestreem != null && filestreem.Length > 0)
                {
                    item.Read(filestreem, 0, filestreem.Length);
                    jsonstr = System.Text.Encoding.Default.GetString(filestreem);
                }
            }
            System.Collections.Generic.Dictionary<string, object> keyValuePairs = null;
            try
            {
                keyValuePairs = Newtonsoft.Json.JsonConvert.DeserializeObject<System.Collections.Generic.Dictionary<string, object>>(jsonstr);
            }
            catch { }
            if (keyValuePairs != null && keyValuePairs.ContainsKey("CurrentVersion"))
            {
                keyValuePairs["CurrentVersion"] = newVersion;
            }
            else if (keyValuePairs != null && !keyValuePairs.ContainsKey("CurrentVersion"))
            {
                keyValuePairs.Add("CurrentVersion", newVersion);
            }
            ...
            jsonstr = Newtonsoft.Json.JsonConvert.SerializeObject(keyValuePairs);
            byte[] savefile = System.Text.Encoding.Default.GetBytes(jsonstr);
            using (var item = File.OpenWrite(AppDomain.CurrentDomain.BaseDirectory + "Versionfile.json"))
            {
                item.Write(savefile, 0, savefile.Length);
            }
        }
